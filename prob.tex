...
% Cálculos derivados
\def\doenca{\prevalencia}
\def\semDoenca{1-\prevalencia}
\def\testePositivoDadoDoenca{\sensibilidade}
\def\testeNegativoDadoDoenca{1-\sensibilidade}
\def\testePositivoDadoSemDoenca{\falsoPositivo}
\def\testeNegativoDadoSemDoenca{\especificidade}

% Cálculo das áreas proporcionais
\def\larguraTotal{12}
\def\alturaTotal{5}
\def\larguraDoenca{\larguraTotal*\prevalencia}
\def\larguraSemDoenca{\larguraTotal*\semDoenca}
\def\larguraVP{\larguraDoenca*\sensibilidade}
\def\larguraFN{\larguraDoenca*\testeNegativoDadoDoenca}
\def\larguraFP{\larguraSemDoenca*\testePositivoDadoSemDoenca}
\def\larguraVN{\larguraSemDoenca*\testeNegativoDadoSemDoenca}
...
\node[disease] (doentes) at (\larguraDoenca/2,2.5) [minimum width=\larguraDoenca cm, minimum height=5cm] {};
\node[no_disease] (nao_doentes) at (\larguraDoenca+\larguraSemDoenca/2,2.5) [minimum width=\larguraSemDoenca cm, minimum height=5cm] {};

\node[font=\small\bfseries, text=white] at (\larguraDoenca/2,4) {Com Doença};
\node[font=\small\bfseries] at (\larguraDoenca/2,3.5) {(1\%)};

\node[font=\small\bfseries] at (\larguraDoenca+\larguraSemDoenca/2,4) {Sem Doença};
\node[font=\small\bfseries] at (\larguraDoenca+\larguraSemDoenca/2,3.5) {(99\%)};

% Divisão horizontal para testes positivos e negativos (metade superior e inferior)
...
% Resultados dos testes dentro do grupo com doença
\node[true_positive] (vp) at (\larguraVP/2,3.75) [minimum width=\larguraVP cm, minimum height=2.5cm] {};
\node[false_negative] (fn) at (\larguraVP+\larguraFN/2,3.75) [minimum width=\larguraFN cm, minimum height=2.5cm] {};

\node[font=\small\bfseries, text=white] at (\larguraVP/2,4.5) {Verdadeiro};
\node[font=\small\bfseries, text=white] at (\larguraVP/2,4.1) {Positivo};
\node[font=\small\bfseries, text=white] at (\larguraVP/2,3.7) {(0,95\%)};

\node[font=\small\bfseries] at (\larguraVP+\larguraFN/2,4.5) {Falso};
\node[font=\small\bfseries] at (\larguraVP+\larguraFN/2,4.1) {Negativo};
\node[font=\small\bfseries] at (\larguraVP+\larguraFN/2,3.7) {(0,05\%)};

% Resultados dos testes dentro do grupo sem doença
\node[false_positive] (fp) at (\larguraDoenca+\larguraFP/2,3.75) [minimum width=\larguraFP cm, minimum height=2.5cm] {};
\node[true_negative] (vn) at (\larguraDoenca+\larguraFP+\larguraVN/2,3.75) [minimum width=\larguraVN cm, minimum height=2.5cm] {};

\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP/2,4.5) {Falso};
\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP/2,4.1) {Positivo};
\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP/2,3.7) {(9,9\%)};

\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP+\larguraVN/2,4.5) {Verdadeiro};
\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP+\larguraVN/2,4.1) {Negativo};
\node[font=\small\bfseries] at (\larguraDoenca+\larguraFP+\larguraVN/2,3.7) {(89,1\%)};

% Total de testes positivos (VP + FP)
\draw[thick, red, dashed] (0,1) -- (\larguraVP,1) -- (\larguraVP,0) -- (\larguraDoenca+\larguraFP,0) -- (\larguraDoenca+\larguraFP,1) -- (\larguraTotal,1);
\node[font=\bfseries, red] at (5.5,-0.5) {Todos os Testes Positivos (10,85\%)};

% Anotações sobre o Valor Preditivo Positivo (VPP)
\draw[annotation, red] (2.5,-0.5) -- (0.5,1);
\node[align=center, font=\small\bfseries, red] at (2.5,-1.2) {VP / (VP + FP) = 0,95\% / 10,85\% $\approx$ 8,76\%};
\node[align=center, font=\small\bfseries, red] at (2.5,-1.8) {Valor Preditivo Positivo (VPP)};
...
\end{tikzpicture}
%\begin{center}
%\includegraphics[width=0.95\textwidth]{bayes_teste_medico.png}

%\vspace{0.3cm}
%\small{Apenas 8,76\% dos testes positivos correspondem a pessoas realmente doentes}
%\end{center}
\end{frame}

% Slide 7: Explicação do Resultado Contra-intuitivo
\begin{frame}
