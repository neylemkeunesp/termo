...
\frame{\frametitle{Definições}
  ...
  \item[Sistema Isolado] Não troca nen energia nem matéria.
...
\frame{\frametitle{Direção da Transmissão de Calor}
...
\frame{\frametitle{Interpretação da Temperatura}
  \begin{center}
    \includegraphics[scale=0.5]{temp}
  \end{center}
Analogia Financeira: Dinheiro é Energia e Entropia é função Utilidade.
}

\frame{\frametitle{Pressão e Volume}
 \begin{center}
...
Se $dV_A>0$ então $p_A>p_B$ ou seja se o volume aumenta isto ocorre porque $p_A>p_B$.
}
...
Mas considere a situação reversa, considere o diferencial:

$$M(x,y)dx+N(x,y)dy$$

Será que existe $G$ tal que $dG$ seja este diferencial?
}
...
\frame{\frametitle{Diferenciais Exatos}
Se temos um diferencial exato então:
...
$$\delta w=-pdV$$
   ...
   \item[Pressão Constante] $w=-p\Delta V$
   \item[Temperatura Constante] 
$$w=-NkT\ln \frac{V_B}{V_A}$$
   \end{description}
 }

\frame{\frametitle{Processo Adiabático}

Neste processo a Entropia não varia.

$dU=\delta W$

Suponha um gás ideal. Neste caso temos que 
...
\begin{tabular}{c c}
  ...
  \begin{itemize}
  \item Expansão Isotérmica
  \item Expansão  Adiabática
  \item Compressão Isotérmica
